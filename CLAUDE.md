<!-- vigiles:sha256:f79d6e711343352b compiled from CLAUDE.md.spec.ts -->
...
- `src/core/dialect.test.ts` — Dialect-port test suite (vitest): defaultDialect is Claude Code, compileAgent verifies the tool contract against the default dialect (built-in ok, typo → did-you-mean), and an INJECTED dialect swaps the catalog (the Codex-prep seam — a tool valid under the alt dialect is flagged under CC)
- `src/adapters/claude-code/dialect.ts` — The Claude Code adapter's HarnessDialect surface: re-exports claudeCodeDialect (canonical def lives in core/dialect.ts as the compiler default) so vigiles/claude-code owns its dialect symmetrically; a Codex adapter defines its own
