<!-- vigiles:sha256:269d995a38255d23 compiled from CLAUDE.md.spec.ts -->
...
- `src/skills-dogfood.test.ts` — Dogfood conformance over vigiles's OWN shipped skills (.claude-plugin/): loadPlugin materializes every skills/<name>/SKILL.md and each has a name + non-empty description (the trigger surface) — the free, model-free floor under the paid trigger-rate eval (a skill that won't load can never fire). Caught a real bug (generate-logo had no frontmatter name). Trigger/precision itself is the eval tier (examples/harness/dogfood/), needs model auth
