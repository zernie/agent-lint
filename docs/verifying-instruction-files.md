...
| ⚙ **Custom rule**     | no off-the-shelf rule fits, but a **custom one CAN** enforce it ("wrap API calls in a retry", "validate the input schema") — doable, not a dead end | the opt-in **synthesis** skill writes + gates a custom rule (with a bit of codebase context), abstaining rather than shipping a checker it can't prove sound; experimental, never auto-run |
