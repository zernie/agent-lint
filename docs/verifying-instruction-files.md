...
**Linter coverage today.** The Enforceable-now lane matches against **ESLint** and **Python / Pylint** — for both it reads your live rule set (including installed **plugins**) and tells a rule that's on from one you documented but left **off**. Ruff and more linters are on the way. One difference remains: if no off-the-shelf rule fits, vigiles can **synthesize a custom rule for JavaScript/TypeScript** (the custom-rule lane's hand-off), but not yet for Python. Everything the map can't match to a rule still gets a lane — hook, custom rule, or judgment call — so nothing is silently dropped.
