...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| 1   | A safety hook that **looks like it blocks but silently doesn't** — wrong exit code, wrong JSON field, or a block decision on an event that can't block (the #1 verified hook pain). | ✦ **Prevented** — a compiled hook emits the protocol for you; you can't write exit-1-for-2 or the wrong field, and "block on a no-decision event" is a type error. | `vigiles/hook`      |
| 2   | An agent that can **read your secrets, take in untrusted text, _and_ send data out** — a prompt-injection data-leak path with no exploit code (the "lethal trifecta").              | ✓ **Caught** (the Safety check, from the tool list alone) · ✦ **prevented** under a typed purity floor.                                                            | audit · lint · spec |
...
| 6   | A **skill that never fires**, or **two skills the agent can't tell apart** so the wrong one runs.                                                                                   | ✓ **Caught** (near-identical descriptions, deterministically) · ◷ **measured** (does it _actually_ fire? recall + precision).                                      | audit · eval        |
...
| 8   | A **hook registered under a misspelled event name** — it never fires.                                                                                                               | ✓ **Caught** (close-typo, high-precision).                                                                                                                         | lint · audit        |
...
- The **static** half of #1 — flag a silent-no-op guard in a **hand-written** hook (so
  `audit` catches it on a plugin that hasn't adopted compiled hooks), with "rewrite it as
  a compiled hook" as the fix.
- A skill that's **invisible** because its frontmatter is missing the opening `---`.
