...
`--deep`-class problem with **no model**, reusing the NCD engine in
...
Skill _triggering_ is normally the model-gated behavioral column (`audit
--deep` / `measureTriggerRate`), which costs tokens. A near-identical
