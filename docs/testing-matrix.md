...
| Mutation runner (`runMutations`: killed vs a neighbour's assertion, survived, no-op edit, ambiguous `find`, a test that was already red, multi-file restore, refusal on a missing test file)                   | unit                     | `src/mutations.test.ts`                                                     |
