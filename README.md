...
And the guard you'd write to stop it usually doesn't work: a widely-copied "safety hook" pattern blocks only **2 of 7** classic dangerous commands in our battery — the other five sail through while the hook _looks_ like it's working. A hook vigiles compiles for you blocks **7 of 7** for two independent reasons: its logic matches the _operation_ (so a reworded `rm -fr` or a quoted flag can't slip past a fragile substring), **and** the exit code and JSON contract are generated (so a `deny` can't silently misfire the way a hand-written `exit 1` or warn-only hook does). Detection is the matching logic; compilation only guarantees the decision actually fires — separate concerns, and you need both.
