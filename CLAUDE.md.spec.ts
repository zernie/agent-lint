...
The inward dependency rule (core ⊄ adapter) is enforced by \`eslint-plugin-boundaries\` (rule \`boundaries/dependencies\`, error-mode, classified by directory) and dogfooded via \`enforce("boundaries/dependencies")\` — the architecture invariant is a verified reference, not a convention. Consumers select a harness by import (\`vigiles/claude-code\` beside the agnostic \`vigiles/testing\`), never a config key; the CLI auto-detects.`,
  ...
  keyFiles: {
    ...
    "src/cli.ts":
      "CLI: init, compile, audit, test, eval, scan (primary commands + generate-types plumbing) + `--version` (prints the version, not the help banner). `init` is the onboarding wizard — interactive at a TTY, non-interactive for agents/CI (or --yes); sets up BOTH pillars by default (spec+types, a vigiles.harness.mjs starter, a zernie/vigiles@v1 workflow), scoped via --verify/--testing (auto-detects the harness; --harness= overrides); adds vigiles to devDependencies (moving a stale `dependencies` pin); installs the Claude Code plugin via the MARKETPLACE (claude plugin install vigiles@vigiles into ~/.claude/plugins/, never vendored into the repo). An existing instruction file gets a scaffolded spec but is never compiled over; only files actually written are listed in the commit hint; a stale old-API CI workflow is flagged loudly, not silently skipped",
    "src/setup-plan.ts":
      "Pure `vigiles init` decision logic (parseSetupArgs/shouldPrompt/resolvePlan): turns CLI flags + whether a human's at a TTY into a SetupPlan {verify,test,gha,plugin,strict}. The two pillars are selected with --verify/--testing (a positive flag selects only it; --no-* drops one from the default-both); the deprecated --pillars=both|verify|test stays as an alias. Interactive when a human runs it, non-interactive (defaults: both pillars) for agents/CI/piped — so 'set up vigiles' from a Claude Code/Codex prompt never hangs. Unit-tested in src/setup-plan.test.ts; the IO (prompts, scaffolds, workflow, plugin marketplace install) stays in cli.ts",
    "src/scan.ts":
