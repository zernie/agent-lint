...
Two boundary rules are enforced by \`eslint-plugin-boundaries\` (rule \`boundaries/dependencies\`, error-mode, classified by directory) and dogfooded via \`enforce("boundaries/dependencies")\`: core ⊄ adapter (the domain stays harness-agnostic), and the \`agnostic-surface\` (\`src/{testing,unit,integration,e2e}.ts\`) ⊄ any adapter (the public agnostic entries route through the composition-root runners, so "agnostic" is enforced, not just named) — the architecture invariant is a verified reference, not a convention. Consumers select a harness by import (\`vigiles/claude-code\` beside the agnostic \`vigiles/testing\`), not a runtime config key; the CLI selects per repo — auto-detect, a \`--harness=\` override, or a \`harness\` key in \`.vigilesrc.json\` written by \`init\` (the project-level declaration, distinct from the library's import-time selection; an array for a repo targeting several harnesses also drives a byte-identical \`CLAUDE.md\`⇄\`AGENTS.md\` mirror when no sync tool fans it out). See \`research/multi-harness-compile.md\`.`,
  ...
  keyFiles: {
    ...
    "src/arg-match.ts":
      "Shared ArgMatcher over a tool call's input (dot-path keys; RegExp = pattern, primitive = exact; AND across keys) + matchesArgs/getPath/stringifyValue/describeArgs/serializeArgs. One matching semantics for both the check vocabulary (toolWith/notTool in src/check.ts) and the tool-fake interception seam (src/tool-fake.ts), so 'did the agent call this tool with these args?' means the same whether you ASSERT on a captured call or INTERCEPT one. Pure + serializable",
    "src/tool-fake.ts":
      "Tool-fake interception — the eval-tier half of the tool-call spy. A FakeTool (tool + optional `when` ArgMatcher + denial reason) is intercepted by an auto-wired PreToolUse hook (exit-2 deny) so a real-model run that DECIDES to hit a paid API / git push / spawn a paid subagent is safe + side-effect-free, while the tool_use (with args) still lands in the Trace for toolWith/notTool. INTERCEPT-AND-PREVENT, not a faithful mock: CC surfaces the deny as a BLOCK not a success, so it's for asserting the ATTEMPT (safety/approval-gate), not stubbing a tool to continue a flow. decideFakeTool (pure, first-match-wins) + buildFakeToolSettings (PreToolUse fragment over the union of faked tool names) + serializeFakeTools/parseFakeTools (VIGILES_FAKE_TOOLS env round-trip, RegExp-safe). The `vigiles fake-tool-hook` CLI subcommand runs the decision. Wired onto the eval spec via EvalArm.fakeTools/MeasureSpec.fakeTools (auto-merged into arm settings, cache keyed on env). See docs/eval-architecture.md",
    "src/tool-fake.test.ts":
      "Tool-fake test suite (vitest): decideFakeTool (unconditional/when-scoped/first-match-wins/default reason), fakeToolHookDecision (PreToolUse event parse + malformed tolerance), buildFakeToolSettings (matcher = escaped union of tool names), serializeFakeTools/parseFakeTools round-trip incl. RegExp survival + junk tolerance. Pure, model-free",
    "src/judge.ts":
