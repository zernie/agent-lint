...
Two boundary rules are enforced by \`eslint-plugin-boundaries\` (rule \`boundaries/dependencies\`, error-mode, classified by directory) and dogfooded via \`enforce("boundaries/dependencies")\`: core ⊄ adapter (the domain stays harness-agnostic), and the \`agnostic-surface\` (\`src/{testing,unit,integration,e2e}.ts\`) ⊄ any adapter (the public agnostic entries route through the composition-root runners, so "agnostic" is enforced, not just named) — the architecture invariant is a verified reference, not a convention. Consumers select a harness by import (\`vigiles/claude-code\` beside the agnostic \`vigiles/testing\`), not a runtime config key; the CLI selects per repo — auto-detect, a \`--harness=\` override, or a \`harness\` key in \`.vigilesrc.json\` written by \`init\` (the project-level declaration, distinct from the library's import-time selection; an array for a repo targeting several harnesses also drives a byte-identical \`CLAUDE.md\`⇄\`AGENTS.md\` mirror when no sync tool fans it out). See \`research/multi-harness-compile.md\`.`,
  ...
  keyFiles: {
    ...
    "research/adapter-api-design.md":
      "Adapter/plugin API design (core-plus-adapters survey: AI SDK, unplugin, Drizzle, Knex, Testing Library, ESLint, OpenTelemetry, dependency-cruiser, Node exports/semver): grounds the pillar-2 relayout. Verdict — single package with subpath exports; the composition root owns the runners (non-breaking move, since exports hide file paths); select-by-import in the library, string only in the CLI (never resolve a string to a module on a library path); enforce the agnostic-surface ⊄ adapter boundary in eslint; parameterize the conformance kit over ADAPTERS; mind the default-param tree-shaking trap. Companion to code-adapter-architecture.md",
    ...
    "research/divergent-bets.md":
      "Divergent strategic bets (beyond extend-the-pillars), triaged with founder reactions: strong = plugin/skill leaderboard + harness cost/ROI optimizer; explore = sell-to-vendors, compliance/attestation, CI-for-model-upgrades; roadmap = self-improving harness (evolve+proofs); demoted = vigiles-as-MCP-oracle (fold into scan); killed = compiler-not-linter + one-source-many-backends; no = SDK bet (gap closed, see sdk-harness-testing); parked = measure-model×harness",
  ...
  rules: {
    ...
    "smooth-adoption": guidance(
      "`npx vigiles init` must work on first run with zero config and set up BOTH pillars by default: a typed spec + types (the Lint pillar), a vigiles.harness.mjs starter (the Test pillar), a `zernie/vigiles@v1` CI workflow (created when none exists; a stale old-API one is flagged, not skipped), vigiles added to devDependencies, and the Claude Code plugin installed via the MARKETPLACE (`/plugin marketplace add zernie/vigiles` + `/plugin install vigiles@vigiles`, into ~/.claude/plugins/ — never vendored into the repo). Onboarding is interactive at a TTY (asks which pillars / CI / plugin) and NON-INTERACTIVE for agents, CI, or piped input (or with `--yes`) — so 'set up vigiles' from a Claude Code / Codex prompt Just Works without hanging on a prompt; `--lint`/`--test` scope the pillars (`--harness=` overrides detection). After install the agent edits specs automatically — no workflow change required. Start permissive (guidance rules, `require-spec: false` available), tighten over time (`--strict`). Hesitant adopters can use inline mode (`<!-- vigiles:enforce ... -->` comments) without a .spec.ts — see `docs/inline-mode.md`. See `research/adoption-strategy.md` and `docs/agent-setup.md`.",
    ),
