...
The inward dependency rule (core ⊄ adapter) is enforced by \`eslint-plugin-boundaries\` (rule \`boundaries/dependencies\`, error-mode, classified by directory) and dogfooded via \`enforce("boundaries/dependencies")\` — the architecture invariant is a verified reference, not a convention. Consumers select a harness by import (\`vigiles/claude-code\` beside the agnostic \`vigiles/testing\`), never a config key; the CLI auto-detects.`,
  ...
  keyFiles: {
    ...
    "src/core/runtime.ts":
      "HarnessRuntime — the runtime/transport PORT (transport axis): the facts the test tiers need to drive a harness (the agent binary to spawn + the env a no-key mock is reached through: base-URL var, API-key var, dummy key), behind one interface instead of `claude` / `ANTHROPIC_*` literals in harness-test/eval/sandbox. A Codex adapter supplies its own HarnessRuntime",
    ...
    "src/codex.ts":
      "`vigiles/codex` — the OpenAI Codex adapter surface (sibling of vigiles/claude-code): re-exports the Codex ports + codexAdapter + the proven Responses mock (startCodexMock/renderResponsesSSE + codexMockArgs/codexMockEnv). Pillar 2 (harness testing/evals against the real codex binary) is full; pillar 1 compile renderers are partial (CC-shaped until the format-axis renderers land)",
