...
Two boundary rules are enforced by \`eslint-plugin-boundaries\` (rule \`boundaries/dependencies\`, error-mode, classified by directory) and dogfooded via \`enforce("boundaries/dependencies")\`: core ⊄ adapter (the domain stays harness-agnostic), and the \`agnostic-surface\` (\`src/{testing,unit,integration,e2e}.ts\`) ⊄ any adapter (the public agnostic entries route through the composition-root runners, so "agnostic" is enforced, not just named) — the architecture invariant is a verified reference, not a convention. Consumers select a harness by import (\`vigiles/claude-code\` beside the agnostic \`vigiles/testing\`), not a runtime config key; the CLI selects per repo — auto-detect, a \`--harness=\` override, or a \`harness\` key in \`.vigilesrc.json\` written by \`init\` (the project-level declaration, distinct from the library's import-time selection; an array for a repo targeting several harnesses also drives a byte-identical \`CLAUDE.md\`⇄\`AGENTS.md\` mirror when no sync tool fans it out). See \`research/multi-harness-compile.md\`.`,
  ...
  keyFiles: {
    ...
    "src/core/mcp-config.ts":
      "MCP-config verification — verifyMcpServers flags a declared MCP server with neither a command (stdio) nor a url (http/sse): it can't start. Pure + FP-safe (an unambiguous requirement, not a catalog); shared by scan + the mcp-config lint rule",
    ...
    "src/core/mcp-tool.ts":
      "MCP-tool resolution — the moat applied to an MCP tool reference's SERVER (the MCP half of the tool moat; agent-tool-contract checks the built-in half but passes any mcp__* unchecked). verifyMcpToolServers(tools, declaredServers, dialect) flags an mcp__server__tool whose server isn't in the plugin's declared mcpServers. HIGH-PRECISION via three sweep-grounded guards: (1) GATE on a declared set — no .mcp.json → reaches global/project servers, flag nothing (the ananddtyagi mcp__ide__* shape); (2) ALLOWLIST harness built-ins via dialect.knownMcpServers (Claude Code's `ide`); (3) SKIP the plugin-namespaced mcp__plugin_<plugin>_<server>__ form (han's playwright-mcp — ambiguous join, the plugin's own server). mcpToolServer extracts the segment. ONE detector reused by scan + the mcp-tool-resolves lint rule; dialect injected",
    "src/core/mcp-tool.test.ts":
      "MCP-tool detector suite (vitest): a declared server resolves, an undeclared one is flagged, GUARD 1 (no declared set → nothing), GUARD 2 (built-in `ide` allowlisted), GUARD 3 (plugin-namespaced form skipped), Tool(restriction) strip, repeated tool de-duped, mcpToolServer extraction",
    "src/core/linters.ts":
      "Cross-referencing engine (ESLint, Stylelint, Ruff, Clippy, Pylint, RuboCop, Cedar)",
    "src/core/dialect.ts":
      "HarnessDialect — the format/dialect PORT (hexagonal format axis): the harness-specific vocabulary the compiler needs (built-in subagent tool catalog, never-available tools, MCP tool shape, built-in MCP servers via knownMcpServers — the mcp-tool-resolves allowlist, optional/additive, e.g. Claude Code's `ide` — hook events, instruction targets, plugin-root token), behind one interface instead of literals hard-coded in compile.ts. core/dialect.ts defines ONLY the interface — the concrete dialects live in the adapters (claudeCodeDialect in src/adapters/claude-code/dialect.ts), symmetric with the other four ports; the compiler takes one by injection (compileAgent requires options.dialect, no core default). A second harness adds a sibling HarnessDialect in its adapter and injects it. See research/code-adapter-architecture.md",
    ...
    "src/leaderboard.test.ts":
      "Leaderboard test suite (vitest): pure scoreReport penalty weights + clamp + empty-machine=0 + command-only/MCP-only-is-a-real-surface, rankPlugins ordering (healthy above broken) over tmp fixtures, formatLeaderboard rendering",
    ...
    "src/adapters/claude-code/agent-runtime.ts":
      "Agent PreToolUse tool-contract rail — the differentiator that closes the declared-vs-enforced gap (#4740/#21460, SDK #172): tools: is documentation, so a PreToolUse hook (vigiles agent-hook) blocks any tool outside the active subagent's contract. parseAgentTools reads the compiled .md frontmatter (the single source of truth the hook enforces) — a thin wrapper over parseAgentToolList(md, key), the generic comma/array list parser reused for disallowedTools: (the disallowed-tools-contract scan/lint), decidePreToolUse is the pure allow/deny, and .vigiles/active-agent.json tracks the dispatched agent — mirrors the skill Stop-hook (src/adapters/claude-code/skill-runtime.ts)",
    ...
    "docs/rules/agent-tool-contract.md":
      "Rule doc: agent-tool-contract — cross-reference a subagent's tools: rail against the harness tool catalog (the moat). High-precision: flags never-available + close-typo only, never a bare unrecognized (plugin/MCP) tool; default warn, error gates CI; same detector as scan + compileAgent",
    "docs/rules/hook-events.md":
      "Rule doc: hook-events — cross-reference a hook's event name against the harness event catalog (a typo never fires). High-precision: close typos only (frameworks like han extend the event set); object-keyed hooks only (a hooks array is a non-CC format, skipped); default warn; same detector as scan",
    "docs/rules/agent-frontmatter.md":
      "Rule doc: agent-frontmatter — flag a SUBAGENT missing name/description (won't register, no fallback). Skills are NOT checked: per CC docs a SKILL.md needs no frontmatter (name←dir, description←first paragraph), so a frontmatter-less skill still loads — flagging it would be an FP. Catches ananddtyagi's prose-only agents; default warn; same detector as scan",
    ...
    "examples/harness/dogfood/test-harness.trigger.eval.mjs":
      "Dogfood trigger eval — vigiles's OWN test-harness skill: fires on harness-testing requests (recall) AND stays quiet on unrelated coding (precision via irrelevantPrompts), gated by assertTriggerRate({ min, maxFalsePositive }). One of 3 model-invocable shipped skills (test-harness, strengthen, edit-spec — each with a trigger eval here); the user-invoked ones (migrate-to-spec, linter-docs) are covered by the free load gate in src/adapters/claude-code/skills-dogfood.test.ts. Write-don't-run without model auth",
    ...
    "research/subagent-compilation.md":
      "Research + roadmap: compiling typed subagent definitions (agent() → agents/<name>.md) — the real Claude Code frontmatter, the declared-vs-enforced gap (tools: is documentation; PreToolUse hook is the rail, issue #4740/#21460, SDK #172), the empirical no-iterator survey (~100 subagents), prior-art agent contracts, and the prioritized next layers (generated enforcement hook, handoff resolution, trigger-rate for dispatch)",
    ...
    "research/adapter-api-design.md":
      "Adapter/plugin API design (core-plus-adapters survey: AI SDK, unplugin, Drizzle, Knex, Testing Library, ESLint, OpenTelemetry, dependency-cruiser, Node exports/semver): grounds the layer-2 relayout. Verdict — single package with subpath exports; the composition root owns the runners (non-breaking move, since exports hide file paths); select-by-import in the library, string only in the CLI (never resolve a string to a module on a library path); enforce the agnostic-surface ⊄ adapter boundary in eslint; parameterize the conformance kit over ADAPTERS; mind the default-param tree-shaking trap. Companion to code-adapter-architecture.md",
    ...
    "research/standards-conformance.md":
      "Research: cross-tool agent-config standardization (AGENTS.md, SKILL.md, MCP, ACP) — verdict: extend layer 1 to be format-neutral (verify AGENTS.md/SKILL.md/MCP refs) and lead the 'valid is not true' cross-referencing wedge; reject the generic-linter (agnix-lane) pivot",
    ...
    "skills/strengthen/SKILL.md":
      "Strengthen skill (MODEL-INVOCABLE): upgrade guidance() → enforce() by finding existing linter rules. Sharpened description + a dogfood trigger eval so the agent reaches for it on its own without over-firing on general linting",
    "skills/edit-spec/SKILL.md":
      "Edit-spec skill (MODEL-INVOCABLE): change a compiled CLAUDE.md/AGENTS.md by editing its .spec.ts (add/modify/remove a rule, section, command, key file) — the skill the pre-edit hook points at when a direct edit is blocked. Absorbs the former generate-rule skill (rule classification: enforce/check/guidance). Dogfood trigger eval gates recall + precision",
    "skills/linter-docs/SKILL.md":
      "Linter-docs reference skill (user-invoked): a SKILL.md indexing the per-linter reference files (eslint/ruff/pylint/rubocop/stylelint/clippy .md) the strengthen + edit-spec skills read when matching a guidance rule to a real linter rule. Fixes the former packaging smell (a skills/ dir with no SKILL.md)",
  },
