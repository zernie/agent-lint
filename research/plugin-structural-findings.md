...
| **MadAppGang/claude-code** (`frontend/agents/tester.md`)                                            | The agent's `tools:` lists **3 `mcp__chrome-devtools__*` tools that no longer exist** on the live `chrome-devtools-mcp` server: `emulate_cpu` + `emulate_network` (consolidated into a single **`emulate`** tool) and `navigate_page_history` (removed). 24/27 still resolve.                        | These tool references are **dead** — the agent can never call them; the browser-emulation + history-navigation capability the author intended is silently gone (classic MCP "rename rot"). **Only a LIVE cross-check catches it** — the static `mcp-tool-resolves` is silent here (the plugin declares no server) and no other linter checks tool existence at all. | **Found 2026-06-22 by dogfooding `verifyMcpContractTools` (the new live MCP tool-resolution) against the real plugin + the real server. Not yet reported upstream.** |
...
> 🔬 **Live MCP tool-resolution — first dogfood finding (2026-06-22).** The
> MadAppGang `tester.md` chrome-devtools row above was found by `verifyMcpContractTools`
> (`src/core/mcp.ts`), the new capability that starts a declared MCP server and checks
> each referenced `mcp__server__tool` actually exists (the "valid ≠ true" cross-check
> applied to the LIVE server, not just the declared set). It's the harness-native
> cross-check moat in action: a real plugin, pinned months ago, accrued 3 dead tool
> references when `chrome-devtools-mcp` consolidated/renamed its tools — invisible to
> every static linter. Reproduce (needs network for `npx`; tools/list responds without
> launching Chrome): point `verifyMcpContractTools(parseAgentTools(tester.md), { "chrome-devtools": { command: "npx", args: ["-y","chrome-devtools-mcp@latest"] } }, claudeCodeDialect)`.
> Live server (chrome-devtools-mcp, 2026-06-22) exposes 29 tools incl. `emulate`,
