...
| skill-validator (agent-ecosystem) | 174 | Go | ✗ | ✗ | ✗ | ✗ | SKILL.md struct + LLM-judge *content quality* (not trigger). |
...
- **A (catalog cross-ref): ZERO OSS tools.** Nobody queries ESLint/Ruff/Clippy/… to
  ...
  event name?" `ctxlint` is the only one doing *any* reference-existence (paths/scripts),
...
players (BentoLabs/Salus — see the vault) are a *different layer*.
