...
| **Surface-test coverage detection** (`untested-surface`) — flag skills/agents/hooks with no test/eval                                                                                    | ✅     | `src/test-coverage.ts`, surfaced by `vigiles audit` (warn-default); colocation + content-reference, user-invoked exempt; the "is there even a test?" gate beside orphan-docs                               |
