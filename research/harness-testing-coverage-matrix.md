...
- **Unit / static** — isolated, model-free, deterministic. Two flavours: a hook's
  ...
  agent / command files (`vigiles lint` / `refs` — do the cited rules, files,
...
| CLAUDE.md / instruction files                 | ✅ refs verified (`lint`)                             | 🟡 present in context, not behaviour                  | ✅ moves behaviour                                                     |
| Skills — procedure / outcome                  | ✅ loads + `description` (`skills-dogfood`) · 🟡 refs | ✅ scripted `Skill` resolves via `pluginDir`          | ✅ real activation via `pluginDir` arm (off: no skill → on: activates) |
| Subagents (`agents/`)                         | 🟡 refs via `lint` (if marked)                        | 🔴 deterministic hard (nested mock sessions)          | ✅ registers + runs via `pluginDir` + Task (probed)                    |
| Slash commands (`commands/`)                  | 🟡 refs via `lint` (if marked)                        | 🟡 expansion is pre-model (needs mock-prompt capture) | ✅ registers + runs via `pluginDir` + `/cmd` (probed)                  |
| MCP servers                                   | ✅ tool refs verified in `lint` (`vigiles:mcp` mark)  | 🔴 not wired                                          | 🔴 bring-your-own                                                      |
...
_references_ are now verified in `lint` against a live server (the `vigiles:mcp`
...
| **Own-plugin dogfood** — conformance over vigiles's own skills/hooks                                                                                                                     | ✅     | `src/skills-dogfood.test.ts` (every `SKILL.md` loads + has `name`/`description`; `${CLAUDE_PLUGIN_ROOT}` hook scripts exist) — caught a missing skill `name` + a drifted hook path                        |
| **Surface-test coverage detection** (`untested-surface`) — flag skills/agents/hooks with no test/eval                                                                                    | ✅     | `src/test-coverage.ts`, surfaced by `vigiles lint` (warn-default); colocation + content-reference, user-invoked exempt; the "is there even a test?" gate beside orphan-docs                               |
| Eval aggregation — mean ± se, variance, **pass^k**, report                                                                                                                               | ✅     | `src/eval.ts` (`MetricStat.passK` = succeeded every trial, alongside mean ± se)                                                                                                                           |
| **Unified `Trace` + bare predicates** (`usedTool`/`toolCount`/`skillResolved`/`toolUsedWith`/`outputContains`/`hookFired`)                                                               | ✅     | one `Trace` (`toolCalls`+`hooks`+`output`+`turns`+`file`) from `runHarnessTest` + `runEval`'s ctx; predicates are bare (eval reuses them as metrics), `assert*` are thin throw-wrappers                   |
| **`trace.hooks`** — which hook fired + its decision                                                                                                                                      | ✅     | `parseHooks` records it from the CLI's `hook_response` stream events (not inferred from marker files); `hookFired`/`hookBlocked`/`assertHookFired`                                                        |
...
| **MCP refs wired into `lint`** — the `` `vigiles:mcp server#tool` `` mark                                                                                                                | ✅     | `vigiles lint` parses the mark (like `vigiles:symbol`), reads `.mcp.json`, starts the referenced server, and flags missing tools / undeclared servers (exit 2). Only runs when a mark references a server |
...
| Line-coverage tooling on the suite                                                                                                                                                       | ✅     | `npm run coverage` — node 22 native V8 coverage (no extra dep); ~86% lines with `claude` present (the eval path is bench-covered). `npm test` is glob-discovered so a new `*.test.js` can't be forgotten  |
...
5. **MCP harness — reference verification + behavioral wiring.** _The standout
   ...
   into `lint`** — the `` `vigiles:mcp server#tool` `` mark is parsed, the server
   from `.mcp.json` is started, and a missing tool / undeclared server fails the
   lint (exit 2). **Remaining:** an optional typed-spec `mcp("github","issue_write")`
...
- **Skill frontmatter lint** — commoditized: [`skill-validator`](https://playbooks.com/skills/louloulin/claude-agent-sdk/skill-validator),
  ...
  `lint`/`refs` already do. Not worth a dedicated check.
...
11. ✅ **Capture `trace.output` (final answer) and `trace.hooks` (which fired +
    decision).** Both shipped. `parseOutput` reads the terminal `result` event;
    `parseHooks` reads the CLI's `hook_response` stream events (`hook_name`,
    `hook_event`, `exit_code`, `outcome`) — so `trace.hooks` is **recorded, not
    inferred from marker files** (the spike found the CLI already surfaces hook
    firing in `stream-json`, so no mock instrumentation was needed). Eval
    switched to `stream-json` so its `ctx.toolCalls` / `ctx.hooks` populate too.
...
- **Acceptance:** existing `src/harness-test.test.ts` / `src/harness-assert.test.ts`
  ...
  shape; `npm test` + `vigiles test` + `lint` all pass.
