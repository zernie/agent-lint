...
> the pieces already built (`src/rule-inventory.ts`, `compiler/extract-existing.js`,
> `compiler/gate.js`, `compiler/catalog/rule-map.json`) into one pipeline, and fixes the
...
- `compiler/extract-existing.js` — opt-in, exec-based: resolves the repo's REAL enabled
...
### Stage 3 — Synthesize + gate (built: `compiler/gate.js`)
