...
- [`compiler-end-to-end-flow.md`](compiler-end-to-end-flow.md) — the whole rule-compiler pipeline (harvest → segment → classify+reconcile → gate → existing-violations → persist → feedback) after the `agent-rules-compiler` fold into `@vigiles/rule-compiler`. Records the two decisions (existing violations grandfather via ESLint's `eslint-suppressions.json` ratchet, never `eslint-disable`; Stage 0 becomes a bulk rule-harvester/taxonomy) and the built-vs-next map (rule-inventory + extract-existing + classify + 129-intent plugin index done; harvester + baseline writer + opt-in audit power-tier next).
