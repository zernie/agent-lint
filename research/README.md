...
- [`loop-combinator.md`](loop-combinator.md) — _[idea]_ **`loop()` extending the finite railway** with a _checkable_ stop condition: a deterministic `until`-gate (compiles to a `Stop`-hook) + a mandatory `maxIters` cap, so an iterating orchestrator's "until done" is a **compiled gate, not prose** ("a loop is a self-edge node; stop conditions live at the loop level"). The product side of the terminatability / "Prose Isn't Policy for control flow" angle.
