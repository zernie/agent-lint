...
| -------------------- | -------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------ |
| Instruction file     | `CLAUDE.md`                                  | `AGENTS.md` (+`CLAUDE.md` fallback)            | **Verify** both (dialect `instructionTargets`); compile renderer is CC-shaped¹ |
| Skills               | `skills/<n>/SKILL.md` (YAML frontmatter)     | `SKILL.md` (name/description)                  | **Verify** (loader + refs); trigger-rate is pillar 2 (CC only today)           |
| Subagents            | `agents/<n>.md` (YAML frontmatter, `tools:`) | `[agents]` **TOML table** in config            | **Verify** for CC (dir); Codex config-table surfaces **not captured yet**²     |
...
¹ Codex `compile` output is currently CC-shaped — the instruction/skill renderers
behind the dialect are the deferred format-axis work.
² Deferred gaps tracked in `research/codex-prototype-findings.md` (#3–#5).
