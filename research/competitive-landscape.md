...
| **[ctxlint](https://github.com/YawLabs/ctxlint)**         | AI agent context files vs. codebase  | Finds dead references, token bloat, contradictions. Post-hoc validation of markdown — same problem space, different angle from vigiles's compile-from-source approach                                    |
| **[agenteval](https://github.com/lukasmetzler/agenteval)** | Linting + benchmarking + scoring     | Scoring/benchmarking angle vigiles doesn't have. Potentially complementary as a CI quality gate                                                                                                          |

## Category 3: Rule Sync / Portability

| Tool                                                              | Focus                                                                                                                                                  |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **[Ruler](https://github.com/intellectronica/ruler)**             | Single source of truth → auto-distributes. Now 16+ agents (added Codex, Gemini CLI), supports skills, subagents, nested mode                            |
| **[rulesync](https://github.com/dyoshikawa/rulesync)**            | Unified rule management CLI, 15+ AI tools                                                                                                              |
...
| **[block/ai-rules](https://github.com/block/ai-rules)**           | Enterprise multi-agent rule management, 11 agents, YAML config, MCP support                                                                            |
| **[vibe-cli](https://github.com/jinjos/vibe-cli)**                | Unifies rules across Claude/Cursor/Copilot/Gemini                                                                                                      |
| **[ai-rulez](https://github.com/Goldziher/ai-rulez)**             | **NEW.** 19+ tools, 32 builtin domains. Claims "AI-Powered Rule Enforcement" — runtime LLM check, **probabilistic**. Positioning contrast for vigiles  |
| **[ai-rules-sync](https://github.com/lbb00/ai-rules-sync)**       | **NEW.** Symlink-based sync across Cursor, Claude, Copilot, OpenCode, Trae AI, Codex, Gemini, Warp                                                     |
| **[anywhere-agents](https://github.com/yzhao062/anywhere-agents)** | **NEW.** Single config with PreToolUse guards blocking destructive Git/GitHub commands                                                                 |
...
| **[Microsoft Agent Governance Toolkit](https://github.com/microsoft/agent-governance-toolkit)**                                                                                 | Runtime governance infra. Seven-package system (Py/TS/Rust/Go/.NET). Sub-0.1ms per action. Framework-agnostic. Covers OWASP agentic AI top 10                                                      |
| **[AWS Bedrock AgentCore Policy](https://aws.amazon.com/blogs/machine-learning/secure-ai-agents-with-policy-in-amazon-bedrock-agentcore/)**                                     | **NEW (GA Mar 2026).** Cedar policy language intercepts every tool call. Sub-millisecond. Natural language → deterministic policy. **Candidate for 7th cross-referenced linter** in vigiles        |

## Category 5: Static / Formal Verification & Academic (Mid-2026)

New category — work that verifies agent behavior with proofs rather than tests or runtime monitors. All released or published in 2025–2026.

| Project                                                                  | Type      | Focus                                                                                                                                                                                                |
| ------------------------------------------------------------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[AgentProof](https://github.com/NordicAgents/AgentProof)**             | Static    | Workflow graph verification across LangGraph/CrewAI/AutoGen/Google ADK. Six structural checks + temporal safety policies → DFA. Sub-second on 5K-node graphs. **Technique transfers to spec evolution** |
| **AgentVerify** (preprints 202604.1029)                                  | Static    | Compositional formal verification via LTL model checking. Memory integrity + tool-call safety specs. Treats LLM as non-deterministic oracle in a verifiable FSM                                       |
| **[AgentGuard](https://arxiv.org/abs/2509.23864)** (ASE 2025)            | Runtime   | Dynamic Probabilistic Assurance via MDPs. Framework-agnostic Python                                                                                                                                  |
| **ContextCov** (arxiv 2603.00822)                                        | Extractor | Natural-language constraints → executable checks. 46K checks from 723 repos. 99.997% syntax validity (semantic accuracy unpublished). **Validates the vigiles thesis at scale**                       |
| **Compiled AI** (arxiv 2604.05150)                                       | Paradigm  | Compile-time code generation eliminates runtime LLM invocation. 96% completion, 57× token reduction. **Academic backing for the vigiles compile-once thesis**                                         |
| **Blueprint First, Model Second** (arxiv 2508.02721)                     | Paradigm  | Framework for deterministic LLM workflow                                                                                                                                                             |
| **Configuring Agentic AI Coding Tools** (arxiv 2602.14690)               | Survey    | Studied 2,923 repos. Claude Code users employ broadest config mechanisms. Skills are mostly static (not LLM-interpreted at runtime)                                                                  |

## Category 6: Adjacent Workflow / Spec Tools

| Project                                                            | Focus                                                                                                                                                |
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[GitHub Spec Kit](https://github.com/github/spec-kit)**          | **NEW (May 2026).** Official GitHub Spec-Driven Development: Spec → Plan → Tasks → Implement. 30 integrations. Different level — feature specs, not instruction specs |
| **[gh-aw (GitHub Agentic Workflows)](https://github.com/github/gh-aw)** | **NEW (Feb 2026 preview).** Markdown + YAML frontmatter for CI workflows. APM (Agent Package Manager) for shared skills. Design inspiration         |
| **[Sigstore A2A](https://github.com/sigstore/sigstore-a2a)**       | Cryptographic signing of A2A Agent Cards. Relevant pattern for signing compiled vigiles outputs                                                       |

**AGENTS.md governance update.** Moved to AAIF (Linux Foundation) with Anthropic, OpenAI, Block, Google, Microsoft, AWS backing. Active proposals: structured tool permissions in YAML frontmatter (#105), standardized rule format for `.agents/rules/` (#179). If structured/machine-readable AGENTS.md ships, vigiles becomes the natural compiler for the standard.
