...
| **PluginLayout** (done)                           | `.claude-plugin/`, `.claude/settings.json` (JSON), `CLAUDE.md`, `${CLAUDE_PLUGIN_ROOT}`     | `.codex/` + `~/.codex/`, `config.toml` (**TOML**), `AGENTS.md`, `${PLUGIN_ROOT}`                                        | **Add `codexLayout`** — but reveals a gap: the loader assumes a **JSON** settings file; Codex hooks live in **TOML** (`[hooks]`) or `hooks.json`. The layout port needs a "settings parser" notion, not just paths.           |
