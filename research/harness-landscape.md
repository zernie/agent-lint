...
| **frontmatter renderers** (DONE)                  | `CLAUDE.md` + `SKILL.md`/agent YAML (`disable-model-invocation`, `argument-hint`, `tools:`) | `AGENTS.md` (plain md) + `SKILL.md` (`name`/`description`)                                                              | **DONE.** `compileSkill` reads `dialect.skillFrontmatter` (Codex `minimal` = name/description); `compileClaude` is format-neutral (AGENTS.md plain md), CC byte-identical. Subagents not compiled (model mismatch, by design). |
