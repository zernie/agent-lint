...
> For the **consolidated, cross-pillar ranking** of everything (this backlog + pillar 2 + the strategic bets), see [`roadmap.md`](roadmap.md) — the single front door. This doc keeps the detailed pillar-1 idea write-ups.
...
|                                       | #9 Hook validation (**partial**) | Med      | a broken `.claude/settings.json` hook fails silently. **`vigiles scan` already checks hook-script existence** (ok/missing/unresolved); remaining: matcher-regex validity + ordering checks                                                                                                                                                                                                                                                                                                                                          |
...
|                                       | **#13 Sandboxed exec**           | **High** | **moat (2026-06-11): "be good at sandboxing & testing" since the eval-runner space is promptfoo's.** Harness-test tier + **`runHook` unit tier (shipped 2026-06-11)** confine under bwrap; remaining: eval tier + non-Linux (`sandbox-exec`/docker)                                                                                                                                                                                                                                                                                 |
| **Distribution / onboarding**         | #14 60-second scan demo          | Med      | The `vigiles scan` **command** shipped (per-plugin report + multi-plugin health leaderboard, `src/scan.ts` / `src/leaderboard.ts`). What remains is the _funnel artifact_: a `npm run demo` on a deliberately-broken instruction file (renamed symbol, missing MCP tool with a "did you mean?") while truthful refs pass — **pulled from the README 2026-06-12 as not-yet-polished**; needs a tightened, reliably-passing script (+ asciinema/GIF) before re-surfacing. See [`distribution-strategy.md`](distribution-strategy.md). |
...
> **Strategic refocus (2026-06-11).** Evals-as-a-framework is promptfoo's space and not worth chasing — in [`eval-api-landscape.md`](eval-api-landscape.md), both the dataset/scorer parity (Phase D) and the interop bridges (Phase E) are **punted**. Our durable edge on the harness pillar is **cost + safety**: the cheaper tiers (`runHook` / `runHarnessTest` need **no model, no API key** — promptfoo is real-model only and structurally expensive) and **sandboxing untrusted harnesses** (#13). Prioritize being _good at sandboxing & testing_ over eval-runner breadth.
...
> **Status (2026-06-11): partly shipped.** `runHarnessTest` confines an untrusted `plugin`/`pluginDir` under bubblewrap safe-by-default (`src/sandbox.ts`), and **`runHook` now takes `sandbox: "auto" \| "strict"`** to confine a third-party hook command (no egress, `--clearenv` with `opts.env` added back via `setenvArgs`), routed through the injectable `runHookWith` seam. **Remaining:** confine the **eval tier** (`runEval` spawns the real `claude` unconfined), and a **non-Linux** backend (`sandbox-exec` / docker) — today `"auto"`/`"strict"` refuse where bwrap is absent.
