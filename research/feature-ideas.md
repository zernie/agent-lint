...
| **Distribution / onboarding**         | #14 60-second scan demo        | Med      | `npm run demo` runs `vigiles audit` on a deliberately-broken instruction file (renamed symbol, missing MCP tool with a "did you mean?") while truthful refs pass — a high-leverage funnel artifact per [`distribution-strategy.md`](distribution-strategy.md). **Pulled from the README 2026-06-12 as not-yet-polished**; needs a tightened, reliably-passing script (and ideally a recorded asciinema/GIF) before re-surfacing in the README. |
