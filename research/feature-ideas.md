...
The **detailed catalog** of pillar-1 verification feature ideas (#1–#14 below) —
each with its programming analog, the user problem, and an implementation sketch
— plus supporting research at the end. It is reference material, **not** a status
board.

**Status, priority, and what ships next live in [`roadmap.md`](roadmap.md)** (the
single cross-pillar front door); pillar-2 harness coverage is in
[`harness-testing-coverage-matrix.md`](harness-testing-coverage-matrix.md). Several
ideas below already shipped (e.g. #3 dead-enforcement, #5 stale-refs, #6 `init`,
#13 sandbox — partial, and #14's `vigiles scan` command) — roadmap.md tracks
which.
