...
- "What's the committed adoption direction?" → \`adoption-direction.md\` (audit-first, markdown = source of truth).`,
  ...
  keyFiles: {
    ...
    "research/report-view-and-hosted-demo.md": `[active] The shared @vigiles/report-view package (SHIPPED via npm workspaces — the four CI-critical changes: root lockfile regen, pages.yml site build, build-report install-guard, sub-lockfile deletion; + the Tailwind @source gotcha) + the Stage-2 hosted "Grade a repo" demo design of record (Fable brief: zero-click baked report, honest gate/tease, edge cases, funnel)`,
