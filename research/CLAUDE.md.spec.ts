/**
 * Directory-scoped INDEX + map for the `research/` corpus (the internal record).
 *
 * The full project rule set is the ROOT `CLAUDE.md` (compiled from
 * `CLAUDE.md.spec.ts`). This nested spec is the AGENT-FACING MAP of every
 * research doc: one `keyFiles` entry per `research/*.md`, so a session working
 * in `research/` gets a scannable one-line summary of each doc as directory
 * memory — without opening 114 files. Source of truth; `research/CLAUDE.md` is a
 * compiled build artifact (`vigiles compile`), and every `keyFiles` path is
 * compiler-verified to EXIST (a deleted/renamed doc fails the compile).
 *
 * KEEP IN SYNC: the `research-index-complete` dogfood test asserts every
 * `research/*.md` (except `README.md`) appears here — add a doc, add its line.
 * Each doc ALSO carries `status:`/`topic:` frontmatter; query the corpus with
 * `grep -l 'status: superseded' research/*.md` etc.
 */
...
- "What's the committed adoption direction?" → \`adoption-direction.md\` (audit-first, markdown = source of truth).`,
  ...
  keyFiles: {
    ...
    "research/linter-adapter-architecture.md": `[idea] P0 — make a linter a cohesive type-enforced unit (LinterAdapter port + Record<BuiltinLinter> registry + conformance loop), mirroring HarnessAdapter/rule-meta; retires the scattered-registry whack-a-mole`,
    ...
    "research/adoption-goals.md": `[active] Adoption NORTH STAR — the GOALS (G1 integrity gate is the universal floor · G2 richer is invited never forced · G3 fit the team don't fight it · G4 a fair result is shareable + sharing is clean · G5 every path serves the one conversion) + the non-negotiable 'not evil' contract (read never writes · declining is free+remembered · never over-promise · grade others fairly Lighthouse-style · no dark-pattern sharing). The important/durable part, distilled into the gate-first-adoption rule in root CLAUDE.md; separate from HOW (adoption-design.md)`,
    "research/adoption-design.md": `[active] Adoption BUILDABLE SPEC (the HOW behind adoption-goals.md): the priority build items — 1. init gate-only + non-evil spec nudge (SetupPlan.gateOnly + nudge with .vigilesrc.json dismissed flag); 2. Tested + rules→enforced HONESTY (detect own-test signal / gate on detectedLinters); 3. non-interactive audit emits vigiles-report.html + leaderboard action points; 4. LOCAL-RESULT SHARE loop (a. public-remote vigiles.sh/?repo= deep-link now, b. self-contained OG social-card image in the HTML report, c. explicit time-boxed upload later); 5. one opt-in maintainer-added README badge. Plus the reputation-safe grading checklist for the public leaderboard/demo (publish methodology, grade artifacts-not-people, MIT-vendored-or-opted-in only, sort-don't-editorialize) grounded in a 2026-07 ethical-viral-loop research pass (Lighthouse/PageSpeed/Socket/Scorecard mechanics; LinkedIn/FB/Path/Duolingo backlash anti-patterns). Notes the davila7-F reconciliation`,
    "research/adoption-personas.md": `[active] Adoption mapped by TEAM POV — the cases vigiles must support (JS/ESLint flagship · Python/Ruff · existing-rich-harness-non-JS gate-only · newcomer · plugin author · platform/CI · multi-harness · security), each with its recommended path (full init vs gate-only lint-in-CI) + what works today + friction to fix; the two axes (supported-linter? existing-harness?) and the cross-cutting gaps (init invasiveness, Tested-reads-as-failure, rules→enforced-silent-on-non-JS, non-interactive-audit HTML/leaderboard). The throughline: sell the integrity gate to everyone, invite the rest. Companion to cli-command-model.md + adoption-direction.md`,
